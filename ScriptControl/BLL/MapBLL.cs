...
namespace com.mirle.ibg3k0.sc.BLL
{
    public class MapBLL
    {
        ...
        public List<BLOCKZONEQUEUE> loadRequestingBlockQueueBySecIds(List<string> entry_sections)
        {
            List<BLOCKZONEQUEUE> blockZoneQueues = null;
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                blockZoneQueues = blockZoneQueueDao.loadRequestingBlockQueueBySecIds(con, entry_sections);
            }
            return blockZoneQueues;
        }
        ...
        public List<BLOCKZONEQUEUE> loadAllNDayAgoAndFinishBlockQueue(int nDay)
        {
            List<BLOCKZONEQUEUE> blockZoneQueues = null;
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                blockZoneQueues = blockZoneQueueDao.loadAllNDayAgoAndFinishBlockQueue(con, nDay);
            }
            return blockZoneQueues;
        }
        public void removeBlockQueueByBatch(List<BLOCKZONEQUEUE> blockQueue)
        {
            using (DBConnection_EF con = DBConnection_EF.GetUContext())
            {
                blockZoneQueueDao.RemoveByBatch(con, blockQueue);
            }
        }
        ...
        public const string REDIS_BLOCK_CONTROL_HASHKEY = "CURRENT_BLOCK_CONTROL_INFO";
        ...
        public void CreatBlockControlKeyWordToRedis(string vh_id, string block_zone_id, bool can_pass, DateTime statusChangeTime)
        {
            string status_change_time = statusChangeTime.ToString(SCAppConstants.TimestampFormat_17);
            ...
            string set_key_word = string.Format(REDIS_BLOCK_CONTROL_KEY_VHID_BLOCKID, vh_id, block_zone_id);
            string set_value_word = string.Format(REDIS_BLOCK_CONTROL_VALUE_STATUS_CHANGE_TIME_AND_STATUS, status_change_time, status);

            scApp.getRedisCacheManager().HashSetAsync
                (REDIS_BLOCK_CONTROL_HASHKEY, set_key_word, set_value_word, timeOut_5min);
        ...
        public void ChangeBlockControlStatus_Blocking(string vh_id, string block_zone_id, DateTime statusChangeTime)
        {
            string status_change_time = statusChangeTime.ToString(SCAppConstants.TimestampFormat_17);
            string set_key_word = string.Format(REDIS_BLOCK_CONTROL_KEY_VHID_BLOCKID, vh_id, block_zone_id);
            string set_value_word = string.Format(REDIS_BLOCK_CONTROL_VALUE_STATUS_CHANGE_TIME_AND_STATUS, status_change_time, SCAppConstants.BlockQueueState.Blocking);
            scApp.getRedisCacheManager().HashSetAsync
                (REDIS_BLOCK_CONTROL_HASHKEY, set_key_word, set_value_word, timeOut_5min);
            LogCollection.BlockControlLogger.Trace($"change redis block zone status , vh id:{vh_id},block id:{block_zone_id}, block status {SCAppConstants.BlockQueueState.Blocking}");
        }
        public void ChangeBlockControlStatus_Through(string vh_id, string block_zone_id, DateTime statusChangeTime)
        {
            string status_change_time = statusChangeTime.ToString(SCAppConstants.TimestampFormat_17);
            string set_key_word = string.Format(REDIS_BLOCK_CONTROL_KEY_VHID_BLOCKID, vh_id, block_zone_id);
            string set_value_word = string.Format(REDIS_BLOCK_CONTROL_VALUE_STATUS_CHANGE_TIME_AND_STATUS, status_change_time, SCAppConstants.BlockQueueState.Through);

            scApp.getRedisCacheManager().HashSetAsync
                (REDIS_BLOCK_CONTROL_HASHKEY, set_key_word, set_value_word, timeOut_5min);
            LogCollection.BlockControlLogger.Trace($"change redis block zone status , vh id:{vh_id},block id:{block_zone_id},block status {SCAppConstants.BlockQueueState.Through}");
        }
        public async void DeleteBlockControlKeyWordToRedisAsync(string vh_id, string entry_sec_id)
        {
            string set_key_word = string.Format(REDIS_BLOCK_CONTROL_KEY_VHID_BLOCKID, vh_id.Trim(), entry_sec_id);
            bool is_success = await scApp.getRedisCacheManager().HashDeleteAsync
                (REDIS_BLOCK_CONTROL_HASHKEY, set_key_word);
            if (is_success)
            {
        ...
        //    return isBefore;
        //}
        public bool IsBlockControlStatus(string vh_id, string block_zone_id, string queue_status)
        {
            string set_key_word = string.Format(REDIS_BLOCK_CONTROL_KEY_VHID_BLOCKID, vh_id, block_zone_id);
            var blockInfo = scApp.getRedisCacheManager().HashGet(REDIS_BLOCK_CONTROL_HASHKEY, set_key_word);
            string sec_id = string.Empty;
            string status = string.Empty;
        ...
        //    status = string.Empty;
        //    string set_key_word = string.Format(REDIS_BLOCK_CONTROL_KEY_VHID_SECID, vh_id);
