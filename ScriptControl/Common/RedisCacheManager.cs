...
namespace com.mirle.ibg3k0.sc.Common
{
    public class RedisCacheManager
    {
        ...
        public void ListRightPush(string list_key, string values)
        {
            IDatabase db = Database();
            if (db == null) return;
            list_key = $"{productID}_{list_key}";
            Task taskSand = db.ListRightPushAsync(list_key, values);
            UsingCount();
            //taskSand.Wait();
        }
        public void ListRemoveAsync(string list_key, string value)
        {
            IDatabase db = Database();
            if (db == null) return;
            list_key = $"{productID}_{list_key}";
            Task taskSand = db.ListRemoveAsync(list_key, value);
            UsingCount();
        }
        ...
        public bool KeyExpire(string key, TimeSpan timeSpan)
        {
            ...
            if (db == null) return false;
            key = $"{productID}_{key}";
            var value = db.KeyExpire(key, timeSpan);
            UsingCount();
            return value;
        }
