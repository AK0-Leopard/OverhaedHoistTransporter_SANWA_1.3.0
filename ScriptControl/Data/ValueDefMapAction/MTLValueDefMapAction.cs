...
{
    public class MTLValueDefMapAction : IValueDefMapAction
    {
        public const string DEVICE_NAME_MTL = "MTL";
        Logger logger = NLog.LogManager.GetCurrentClassLogger();
        MaintainLift eqpt = null;
        ...
        {
            this.eqpt = baseEQ as MaintainLift;
        ...
        public virtual void MTL_CarOutRequest(object sender, ValueChangedEventArgs args)
        {
            var recevie_function =
                scApp.getFunBaseObj<MtlToOHxC_MtlCarOutRepuest>(eqpt.EQPT_ID) as MtlToOHxC_MtlCarOutRepuest;
            var send_function =
                scApp.getFunBaseObj<OHxCToMtl_MtlCarOutReply>(eqpt.EQPT_ID) as OHxCToMtl_MtlCarOutReply;
            try
            {
                recevie_function.Read(bcfApp, eqpt.EqptObjectCate, eqpt.EQPT_ID);
                LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(MTLValueDefMapAction), Device: DEVICE_NAME_MTL,
                ...
                if (hand_shake == 1)
                {
                    ...
                    {
                        scApp.MTLService.carOutRequestCancle(eqpt);
                    ...
                    else
                    {
                        ...
                        MaintainSpace maintainSpace = scApp.EquipmentBLL.cache.GetMaintainSpace();
                        var car_out_check_result = scApp.MTLService.checkVhAndMTxCarOutStatus(this.eqpt, maintainSpace, pre_car_out_vh);

                        var request_result = scApp.MTLService.carOutRequset(eqpt, pre_car_out_vh_num);
                        send_function.ReturnCode = request_result.isSuccess ? (ushort)1 : (ushort)2;
                        LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(MTLValueDefMapAction), Device: DEVICE_NAME_MTL,
                                 Data: $"Process MTL car out request, is success:{request_result.isSuccess},result:{request_result.result}",
                                 VehicleID: eqpt.EQPT_ID);
                ...
                {
                    AVEHICLE pre_car_out_vh = scApp.VehicleBLL.cache.getVhByNum(pre_car_out_vh_num);
                    scApp.MTLService.processCarOutScenario(eqpt, pre_car_out_vh);
                }
        ...
        public virtual void MTL_CarInRequest(object sender, ValueChangedEventArgs args)
        {
            var recevie_function =
                  scApp.getFunBaseObj<MtlToOHxC_RequestCarInDataCheck>(eqpt.EQPT_ID) as MtlToOHxC_RequestCarInDataCheck;
            var send_function =
                scApp.getFunBaseObj<OHxCToMtl_ReplyCarInDataCheck>(eqpt.EQPT_ID) as OHxCToMtl_ReplyCarInDataCheck;
            try
            {
                recevie_function.Read(bcfApp, eqpt.EqptObjectCate, eqpt.EQPT_ID);
                LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(MTLValueDefMapAction), Device: DEVICE_NAME_MTL,
                ...
                {
                    var check_result = scApp.MTLService.checkVhAndMTxCarInStatus(eqpt, null, pre_car_in_vh);
        ...
        public virtual void MTL_LFTStatus(object sender, ValueChangedEventArgs args)
        {
            var recevie_function =
                scApp.getFunBaseObj<MtlToOHxC_LFTStatus>(eqpt.EQPT_ID) as MtlToOHxC_LFTStatus;
            try
            {
                recevie_function.Read(bcfApp, eqpt.EqptObjectCate, eqpt.EQPT_ID);
                LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(MTLValueDefMapAction), Device: DEVICE_NAME_MTL,
                         Data: recevie_function.ToString(),
                         VehicleID: eqpt.EQPT_ID);
                eqpt.HasVehicle = recevie_function.HasVehicle;
                eqpt.StopSingle = recevie_function.StopSingle;
                eqpt.MTxMode = (ProtocolFormat.OHTMessage.MTxMode)recevie_function.Mode;
                eqpt.MTLLocation = (ProtocolFormat.OHTMessage.MTLLocation)recevie_function.LFTLocation;
                eqpt.MTLMovingStatus = (ProtocolFormat.OHTMessage.MTLMovingStatus)recevie_function.LFTMovingStatus;
                eqpt.Encoder = recevie_function.LFTEncoder;
                eqpt.VhInPosition = (ProtocolFormat.OHTMessage.VhInPosition)recevie_function.VhInPosition;
            }
            catch (Exception ex)
            {
                logger.Error(ex, "Exception");
        ...
        public virtual void CarInSafetyChcek(object sender, ValueChangedEventArgs args)
        {
            var recevie_function =
                scApp.getFunBaseObj<MtlToOHxC_CarInSafetyCheck>(eqpt.EQPT_ID) as MtlToOHxC_CarInSafetyCheck;
            try
            {
                recevie_function.Read(bcfApp, eqpt.EqptObjectCate, eqpt.EQPT_ID);
                LogHelper.Log(logger: logger, LogLevel: LogLevel.Info, Class: nameof(MTLValueDefMapAction), Device: DEVICE_NAME_MTL,
                         Data: recevie_function.ToString(),
                         VehicleID: eqpt.EQPT_ID);
                eqpt.CarInSafetyCheck = recevie_function.SafetyCheck;
        ...
        public virtual void CSTInterface(object sender, ValueChangedEventArgs args)
        {
            try
            {
