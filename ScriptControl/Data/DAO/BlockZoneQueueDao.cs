...
namespace com.mirle.ibg3k0.sc.Data.DAO
{
    public class BlockZoneQueueDao
    {
        ...
        public void RemoveByBatch(DBConnection_EF con, List<BLOCKZONEQUEUE> cmd_mcss)
        {
            cmd_mcss.ForEach(entity => con.Entry(entity).State = EntityState.Deleted);
            con.BLOCKZONEQUEUE.RemoveRange(cmd_mcss);
            con.SaveChanges();
        }
        ...
        public List<BLOCKZONEQUEUE> loadRequestingBlockQueueBySecIds(DBConnection_EF con, List<string> sec_ids)
        {
            var query = from block in con.BLOCKZONEQUEUE
                            //where sec_ids.Any(sec => block.ENTRY_SEC_ID.Contains(sec.Trim()))
                        where sec_ids.Contains(block.ENTRY_SEC_ID.Trim())
        ...
        public List<BLOCKZONEQUEUE> loadAllNDayAgoAndFinishBlockQueue(DBConnection_EF con, int nDay)
        {
            DateTime ReferenceTime = DateTime.Now.AddDays(-nDay);
            var query = from block in con.BLOCKZONEQUEUE
                        where (block.STATUS.CompareTo(SCAppConstants.BlockQueueState.Release) >= 0)
                           && block.REQ_TIME < ReferenceTime
                        select block;
            return query.ToList();
        }
