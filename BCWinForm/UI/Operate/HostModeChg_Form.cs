...
namespace com.mirle.ibg3k0.bc.winform.UI
{
    public partial class HostModeChg_Form : Form
    {
        ...
        private void butOnlineRemote_Click(object sender, EventArgs e)
        {
            ...
            if (!scApp.LineService.canOnlineWithHost(out not_ready_vh_id))
            {
                Common.LogHelper.Log(logger: NLog.LogManager.GetCurrentClassLogger(), LogLevel: LogLevel.Info, Class: nameof(HostModeChg_Form), Device: "OHTC",
                  Data: $"Want to send online remote with mcs,but has vh not ready");
                MessageBox.Show($"Has vh not ready,vh:{not_ready_vh_id}");
            }
            else if (scApp.getEQObjCacheManager().getLine().Host_Control_State == HostControlState.On_Line_Remote)
            {
                Common.LogHelper.Log(logger: NLog.LogManager.GetCurrentClassLogger(), LogLevel: LogLevel.Info, Class: nameof(HostModeChg_Form), Device: "OHTC",
                  Data: $"Want to send online remote with mcs,but with host online ready");
                MessageBox.Show("On line ready");
            }
            else
            {
                Common.LogHelper.Log(logger: NLog.LogManager.GetCurrentClassLogger(), LogLevel: LogLevel.Info, Class: nameof(HostModeChg_Form), Device: "OHTC",
                  Data: $"Send online remote with mcs...");
        ...
        private void butOffline_Click(object sender, EventArgs e)
        {
            ...
            {
                Common.LogHelper.Log(logger: NLog.LogManager.GetCurrentClassLogger(), LogLevel: LogLevel.Info, Class: nameof(HostModeChg_Form), Device: "OHTC",
                  Data: $"Want to send offline with mcs,but current sc state not paused,current state:{scApp.getEQObjCacheManager().getLine().SCStats}");
            ...
            {
                Common.LogHelper.Log(logger: NLog.LogManager.GetCurrentClassLogger(), LogLevel: LogLevel.Info, Class: nameof(HostModeChg_Form), Device: "OHTC",
                  Data: $"Want to send offline remote with mcs,but with host offline ready");
                MessageBox.Show("Current is off line");
            }
            else
            {
                Common.LogHelper.Log(logger: NLog.LogManager.GetCurrentClassLogger(), LogLevel: LogLevel.Info, Class: nameof(HostModeChg_Form), Device: "OHTC",
                  Data: $"Send offline with mcs...");
        ...
        {
            Common.LogHelper.Log(logger: NLog.LogManager.GetCurrentClassLogger(), LogLevel: LogLevel.Info, Class: nameof(HostModeChg_Form), Device: "OHTC",
              Data: $"Send change sc state to auto...");
        ...
        {
            Common.LogHelper.Log(logger: NLog.LogManager.GetCurrentClassLogger(), LogLevel: LogLevel.Info, Class: nameof(HostModeChg_Form), Device: "OHTC",
              Data: $"Send change sc state to pause...");
